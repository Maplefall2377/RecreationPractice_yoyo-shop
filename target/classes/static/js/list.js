...
function getData(type, page){
    let size = 4
    $.get('/api/index/recommendPage', 'type='+type+'&page='+page+'&size='+size, function (result){
        if(result.code == 0){
            // 要展示的数据
...
let typeId = urlParams.has('typeId') ? urlParams.get('typeId') : '2';
// 判断点击的那个项
if(typeId == '3'){
    // 新品
